...
Clinical = pd.read_excel('data_clinical_supp_patient.xlsx')
GeneData = pd.read_excel('data_expression.xlsx',header=None)
GeneTr= GeneData.T
...
CombinedData = pd.merge(GeneTr, Clinical, how="left", left_on='PATIENT_ID', right_on='PATIENT_ID')
'PATIENT_ID' in CombinedData

##CombinedData.to_csv('combined_data.csv')


X1 = CombinedData.ix[:,1:24367]
Y = CombinedData['VITAL_STATUS']

#IMPLEMENT FEATURE SELECTION - UNIVARIATE METHOD
test = SelectKBest(score_func=chi2, k=500)     # DEFINE THE NUMBER OF FEATURES IN K
